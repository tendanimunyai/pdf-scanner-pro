# PDF Scanner Pro

A mobile document scanner that captures pages with a phone camera, extracts text
with OCR, and exports searchable PDF files.

See [REQUIREMENTS.md](REQUIREMENTS.md) for the product scope, technical
requirements, and acceptance criteria.

## Project documentation

- [Architecture](docs/ARCHITECTURE.md)
- [MVP delivery plan](docs/MVP_DELIVERY_PLAN.md)
- [Privacy and data flow](docs/PRIVACY_AND_DATA_FLOW.md)
- [Technical spikes](docs/TECHNICAL_SPIKES.md)
- [Test strategy](docs/TEST_STRATEGY.md)
- [Permissions matrix](docs/PERMISSIONS_MATRIX.md)
- [Local development](docs/LOCAL_DEVELOPMENT.md)
- [Dependency and license review](docs/DEPENDENCY_AND_LICENSE_REVIEW.md)
- [UX flows](docs/UX_FLOWS.md)
- [Release and rollback](docs/RELEASE_AND_ROLLBACK.md)
- [Google Play discovery and local measurement](docs/GOOGLE_PLAY_ASO.md)
